...
xx.module("myExpl", function (apod) {
  ...
  function showTheFolders() {
    return new Promise((resolve, reject) => {
      ...
      folders.getFob().then(() => {
        ...
        resolve(); // this also displays the folders and files
      });
    });
  }
  ...
  function jsFolders(json, p, level) {
    ...
    while (true) {
      if (json.ps[p] > 0) {
        ...
        s += `<div id="ai_${p}">
          <span class="indx">${common.getHtml(
          "plusMinus"
        )}</span><span class="holder"><span 
  ...
  q._("itemPopup").addEventListener("click", function __clickItemPopup(e) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let data = { dir: fp };
      common.doFetch("ft_Explorer.php", "getFiles", null, data, "", function (json) {
  ...
  // function add event handlers
  function addEventHandlers(edu) {
    q.delegate("files_div", "click", "li", function __clickOnFilename(e) {
      ...
      if (fname != selected_file) {
        selected_file = fname;
        // q.removeAllClasses("#files_div li span", "selected");
