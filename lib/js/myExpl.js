...
xx.module('myExpl', function (apod) {
  ...
  delegate('folders_div', 'mouseover', '.holder', function __mouseoverOnHolder(e) {
    //e.stopPropagation();
  ...
  delegate('files_div', 'click', 'li', function __clickOnFilename(e) {
    fname = this.dataset.fp;
    if (fname != selected_file) {
      selected_file = fname;
      removeAllClasses('#files_div li', 'selected');
      this.classList.add('selected');
      _('fn_input').value = fname.substring(fname.lastIndexOf('/') + 1);
    }
    singleclickdone = true;
  });

  delegate('files_div', 'dblclick', 'li', function __dblclickOnFilename(e) {
    if (singleclickdone === true) {
      edu.checkopenfile('Open', fname);
    }
  });

  delegate('myExplorer', 'click', 'button', function __clickButtonInMyExplorer(e) {
  ...
  class foldersClass {
    ...
    putFob() {
      ...
      if (displaceLeft) { 
        _('folders_div').querySelector('.items').style.marginLeft = -14 + 'px'; 
      }
