...
xx.module('edu', function (apod) {
  ...
  function readyFunction() {
    ...
    // showDialog('startupbox');}),3000);  // for testing loading div
  ...
  function fixCompid() {
    if (typeof (localStorage) === "undefined") {
      ...
      // common.doAjax('general.old.php', 'setCompID', {}, '', function (json) {
      common.doFetch('ft_temp.php', 'setCompID', null, {}, '', function (json) {
      ...
      // common.doAjax('general.old.php', 'setSessionCompID', { 'compID': xx.vars.compid }, '', function (json) {
      common.doFetch('ft_temp.php', 'setSessionCompID', null, { 'compID': xx.vars.compid }, '', function (json) {
        if (json.value !== 'success') {
          if (json.errmsg != undefined) {
            if (vars.debugon) alert(json.errmsg);
          } else {
  ...
  function initTopVariables() {
    ...
    // q._('siteName').textContent = meta.siteVars.siteName;
    // q._('pageName').textContent = meta.siteVars.pageName;
    ...
    // let sw = $topdiv.find('#siteName').width();
    // let pw = $topdiv.find('#pageName').width();

    // let elem = q._('topdiv');
    // let siw = elem.querySelector('#siteName').offsetWidth;
    // let piw = elem.querySelector('#pageName').offsetWidth;

    // width of top menu items and the filename box
    // topscorew = $('#scorecontainer').outerWidth();
    // topscorew = q._('scorecontainer').offsetWidth;
    topscorew = q.offsetWidth(q._('scorecontainer'));
    ...
    // setWidth(topmenumask, topmenuw);
    topmenumask.style.width = topmenuw + 'px';
    // setHeight(topmenumask, theMenu.offsetHeight);
    ...
    // $('#toprightmenumask').outerWidth(topmenuw - $(theMenu.firstChild).outerWidth()).outerHeight(theMenu.offsetHeight);
    const toprightmenumask = q._('toprightmenumask');
    // setWidth(toprightmenumask, topmenuw - theMenu.firstChild.offsetWidth);
    // setHeight(toprightmenumask, theMenu.offsetHeight);
    toprightmenumask.style.width = (topmenuw - theMenu.firstChild.offsetWidth) + 'px';
    ...
    // $('#gamecontainermask').outerWidth(topgobuttonw).outerHeight(gobutt.offsetHeight);
    const gamecontainermask = q._('gamecontainermask');
    // setWidth(gamecontainermask, topgobuttonw);
    // setHeight(gamecontainermask, gobutt.offsetHeight);
    ...
    q._('dialogmask').style.top = topdiv.offsetHeight + 'px';
  }
  ...
  function initAudio() {
    ...
    function initAud(c, b, r) {
      ...
      if (c === "cont") {
        ...
        q._('cont').addEventListener('click', e => { slider.moveBall(e); setVolume(); });
      } else {
        // $("#" + c).on("click", e => { slider.moveBall(e); });
        q._(c).addEventListener('click', e => { slider.moveBall(e); });
      }
      return slider;
    }
  }
  ...
  function putMask(mask, container) {
    ...
    const tmp = q._(mask);
    tmp.style.width = c.offsetWidth + 'px';
    tmp.style.height = c.offsetHeight + 'px';
    tmp.classList.remove('nodisplay');
  }
  ...
  function centreDialog(dialog) {
    ...
    if (wrap.offsetHeight < h) {
      ...
      myCheck(2, 'increase wrap.offset+Height');
  ...
  function checkWidth() {
    ...
    // let bw = Math.round(window.innerWidth);   // this is the display width, vertical scrollbar not included
  ...
  function testKeyCode(e) {
    ...
    if (e.altKey) {
      ...
      if (keycode === 83) {   // Alt-S was pressed, toggles vars.debugon
        ...
        (vars.debugon) ? q._('debug').style.display = '' : q._('debug').style.display = 'none';
  ...
  // q.qs('#startupbox button').addEventListener('click', function clickOnStartupboxButton() {
  ...
  q.delegate('unlockbox', 'click', 'button', function clickOnUnlockboxButton() {
    ...
    if (butt === 'Next') {
      ...
      // let tmp = q.qa('#unlockbox input');
      // let val1 = tmp[0].value;
      ...
      if (correctKey(ob)) {
        ...
        // common.doAjax('general.old.php', 'setFree', { 'doCookie': doCookie }, '', function (json) {
        common.doFetch('ft_temp.php', 'setFree', null, { 'doCookie': doCookie }, '', function (json) {
          isRestricted = false;
          common.showMessage({
            msg: 'Correct key', bc: 'g', callback: function () {
  ...
  function OpenFile() {
    ...
    // document.getElementById('myExplorer').classList.remove('nodisplay');
  }
  ...
  //   if (fn === '') {
  //     q._('toprightmenumask').classList.remove('nodisplay');
  ...
  //     // common.doAjax('general.old.php', 'openfile', data, 'Reading the file, please wait', function (json) {
  //     common.doFetch('ft_temp.php', 'openfile', null, data, 'Reading the file, please wait', function (json) {
  //       if (json.value === 'success') {
  ...
  //           q._('backQTime_sound').setAttribute('src', backgroundQTimeSound);
  ...
  function getandopenfile(butt) {
    return new Promise((resolve, reject) => {
      ...
      if (butt === "Open") {
        ...
        let tmp = root + fname1.substr(0, fname1.lastIndexOf('.')) + '_files/';
        // if (filesDir === tmp) return dfd.resolve('cancel');
        if (filesDir === tmp) resolve('cancel');
        filesDir = tmp;
        var stored = null;
        if (typeof (localStorage) !== "undefined") {
          stored = localStorage.getItem(fname1);
        ...
        common.doFetch('ft_temp.php', 'openfile', null, data, 'Reading the file, please wait', function (json) {
          if (json.value === 'success') {
            ...
            topPageNamew = ele.offsetWidth;
            // putTopdiv($('body').width());
            putTopdiv(getComputedStyle(document.body, null).width.replace("px", ""));
            document.getElementById('fn_output').value = fn;
            // get total qs
            // dfd.resolve('good');
            resolve('good');
  ...
  q.delegate(document.body, 'keyup', '#gnq2_input', function (ee) {
    ...
    if (badInput(v)) q._('gnq2_input').value = orgnq; else orgnq = v;
  });
  ...
  q.delegate(document.body, 'keyup', '#gta2_input', function (ee) {
    ...
    if (badInput(v)) q._('gta2_input').value = orgta; else orgta = v;
  });
  ...
  q._('gobutton').addEventListener('click', function __clickOnGoButton__(e) {
    ...
    if (gamestate === 'New game') {
      ...
      if (fillTestQ(testQ, ob)) {
        totalSelected = ob.n;
        if (totalSelected === 0) {
          common.showMessage({ msg: 'There are no questions in the selection pool', bc: 'w' });
          return;
        }
        // disable the menu
        tm_disable();
        ...
        Promise.all([initForTest(), getnextq()]).then(function (result) {
          common.loadingOff();
          if (result === 'end') {
            common.showMessage({ msg: 'There are no valid questions in the selection pool', bc: 'w' });
            tm_enable();
            return;
          }
  ...
  function doresult(s, ss = 'Wrong') {
    ...
    ts.setAttribute('style', `font-size:1.3em;z-index:400;color:${common.rgbaColor(tickColor)};position:absolute;top: ${tickpos}px;left:36px;`);
    ts.textContent = '\u2714';
    // ts.textContent = 'style="font-size:1.3em;z-index:400;color:' + common.rgbaColor(tickColor) + ';position:absolute;top:' + tickpos + 'px;left:36px;">\u2714';
  ...
  function putBackgroundImage() {
    ...
    q._('wrapper').style.backgroundImage = 'url(' + filesDir + pictureShowing + ')';
  ...
  q._('wrappermask').addEventListener('click', function __clickOnWrappermask(e) {
    ...
    getnextq_dfd.then(function (result) {
      common.loadingOff();
      if (result === 'end') {
        common.showMessage({ msg: 'Unable to complete the test - some questions had errors', bc: 'w' });
        mostate = -1;
        q._('thescore').style.backgroundColor = 'red';
        return;
      }
  ...
  function flashmsg(msg1, ms, bc) {
    ...
    centreBoxAndShow(fb);
    //ms = 20000;
    setTimeout(() => { fb.classList.add('nodisplay'); }, ms);
  }

  function centreBoxAndShow(b) {
    // b is a jS element
    // const $b = $(b);
// let tmp = $("body").scrollTop();
// tmp = $("body").scrollLeft();
...
// tmp = document.body.scrollLeft;
    ...
    let t = (window.innerHeight - q.offsetHeight(b)) / 2.7 + document.body.scrollTop;
    let l = (window.innerWidth - q.offsetWidth(b)) / 2 + document.body.scrollLeft;
...
// if less than a certain value then make them a minimum value
  ...
  function getqna(q, ob, qx) {
    return new Promise((resolve, reject) => {
      ...
      if (nextPic != '') {
        ...
        //   }
        // });
        // getImageSize(qx, 7).then(function () {
        //   // don't worry about setting ok here, it is only the background image
        //   if (--todocount === 0) {
        //     if (ok) $dfd.resolve(); else $dfd.reject();
        //   }
        ...
        //   }
        // });
        // getImageSize(qx, 7).finally(function (result) {
      ...
      for (let i = 0; i <= ob.nOps; i++) {
        if (qx.picPath[i] != '') {
          ...
          //   alert('error from getimagesize')
          //   if (--todocount === 0) $dfd.reject();
          // });
          getImageSize(qx, i).then(function (result) {
  ...
  function setVolume() {
    ...
    // let arr = Array.from(qa('#clips0 audio'));
    // for(let ele of arr) ele.volume = vol;
    // for(let ele of Array.from(qa('#clips1 audio'))) ele.volume = vol;

    // for(let ele of q.qa('#clips0 audio')) ele.volume = vol;
    // for(let ele of q.qa('#clips1 audio')) ele.volume = vol;

    q.qa('#clips0 audio').forEach((ele) => { ele.volume = vol; });
    q.qa('#clips1 audio').forEach((ele) => { ele.volume = vol; });
  }
