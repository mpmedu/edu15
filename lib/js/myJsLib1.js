...
function getMargin(ele){
  ...
  //return parseFloat(getComputedStyle(ele, null).margin.replace("px", ""));
  //let m = getComputedStyle(ele, null).margin;
  //let marr = m.split(' ');
  //marr = marr.map(v => parseFloat(v));
  //return m.split(' ').map(v => parseFloat(v));
  return getComputedStyle(ele, null).margin.split(' ').map(v => parseFloat(v));
  //return parseFloat(getComputedStyle(ele, null).margin);
}
